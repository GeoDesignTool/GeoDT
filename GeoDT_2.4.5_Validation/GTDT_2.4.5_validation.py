...
class cauchy: #functions modified from JPM
    def __init__(self):
        self.sigP = np.zeros((3,3),dtype=np.float)
        self.sigG = np.zeros((3,3),dtype=np.float)
        self.Sh = 0.0 #Pa
        self.SH = 0.0 #Pa
        self.SV = 0.0 #Pa
...
class reservoir:
    def __init__(self):
        ...
        self.fNum = np.asarray([10,
                                10,
                                10],dtype=np.int) #count 
        self.fDia = np.asarray([[300.0,900.0],
                                [300.0,900.0],
                                [300.0,900.0]],dtype=np.float) #m
        self.fStr = np.asarray([[79.0*deg,8.0*deg],
                                [0.0*deg,8.0*deg],
                                [180.0*deg,8.0*deg]],dtype=np.float) #m
        self.fDip = np.asarray([[60.0*deg,8.0*deg],
                                [90.0*deg,8.0*deg],
                                [0.0*deg,8.0*deg]],dtype=np.float) #m
...
class nodes:
    ...
    def __init__(self):
        ...
        self.p = np.zeros(self.num,dtype=np.float)
        self.T = np.zeros(self.num,dtype=np.float)
        self.h = np.zeros(self.num,dtype=np.float)
#        self.f_id = [[-1]]
    #add a node
...
#                    vtkFile=(fname + '_pipes.vtk'))
    ...
    def build_pts(self,spacing=25.0,fname='test_gridx'):
        ...
        data = np.ones((num,num,num),dtype=np.float)*self.rock.BH_T
...
#        hold = -1*np.ones(len(self.faces),dtype=np.int)
...
#    def get_flow(self,p_bound=0.0*MPa,q_inlet=[],q_outlet=[],p_inlet=[],p_outlet=[],reinit=True):
    def get_flow(self,p_bound=0.0*MPa,q_well=[],p_well=[],reinit=True,useprior=False):
...
#        self.set_bcs(p_bound=p_bound,q_inlet=q_inlet,q_outlet=q_outlet,p_inlet=p_inlet,p_outlet=p_outlet)
        ...
        i_q = []
        i_p = []
        p_q = np.zeros(len(self.wells),dtype=np.float)
        p_p = np.zeros(len(self.wells),dtype=np.float)
        ...
        if len(b_pipes) > 0:
            for w in b_pipes:
                b_q += [-self.q[w]]
        b_p = list(np.ones(len(b_q),dtype=np.float)*b_p[0])
    ...
    def get_heat(self,plot=True,
        ...
        x = np.linspace(T5,Tr,100,dtype=np.float)
        y = np.zeros(100,dtype=np.float)
...
#        Y = self.pipes
        ...
        ht = np.zeros((t_n+1,N),dtype=np.float)
        Tt = np.zeros((t_n+1,N),dtype=np.float)
        Rt = np.zeros((t_n+1,Np),dtype=np.float)
        Et = np.zeros((t_n+1,Np),dtype=np.float)
        Qt = np.zeros((t_n+1,Np),dtype=np.float)
        Er = np.zeros(Np,dtype=np.float)
        ...
        for t in range(0,len(ts)-1):
            ...
            while 1:
                ...
                Ap = np.zeros(Np,dtype=np.float)
                Bp = np.zeros(Np,dtype=np.float)
                Cp = np.zeros(Np,dtype=np.float)
                Dp = np.zeros(Np,dtype=np.float)
                Ep = np.zeros(Np,dtype=np.float)
                hm = np.zeros(N,dtype=np.float)
                mi = np.zeros(N,dtype=np.float)
                for p in range(0,Np):
                    #working variables
                    #u = self.pipes.fID[p]
                    n0 = self.pipes.n0[p]
...
#                    dh = 0.0
                ...
                z = []
                z = np.zeros(N,dtype=np.float) #-0.0001*np.random.rand(N) + Tr #K
                hu = np.zeros(N,dtype=np.float)
                for n in range(0,N):
...
#                    hu[Tb[j][0]] = therm(T=z[Tb[j][0]],P=Pn[Tb[j][0]]).h
        ...
        #thermal energy extraction
        dht = np.zeros(len(ts),dtype=np.float)
    ...
    def dyn_stim(self,Vinj = -1.0, Qinj = -1.0, dpp = -666.6*MPa,
        ...
        for i in range(0,len(self.wells)):
            ...
            if (int(self.wells[i].typ) in [typ('producer')]):
                p_key += [int(i)]
                p_div += 1
        i_key = np.asarray(i_key,dtype=np.int)
        p_key = np.asarray(p_key,dtype=np.int)
        ...
        vol_new = 0.0 #m3
        vol_old = 0.0 #m3
        vol_rem = np.ones(i_div,dtype=np.float)*Vinj #m3
        Pis = [] #Pa
        Qis = [] #m3/s
        completed = np.zeros(i_div,dtype=np.bool) #T/F segment stim complete
        hydrofrac = np.zeros(i_div,dtype=np.bool) #T/F hydrofrac instability detection
        stabilize = np.zeros(i_div,dtype=np.bool) #T/F detection of stable flow
        tip = np.ones(i_div,dtype=np.float)*bhp #trial injection pressure
        dpi = np.ones(i_div,dtype=np.float)*-self.rock.dPi #trial pressure incrementer
...
#            print Qi
            ...
            face_pmax = np.ones(len(self.faces),dtype=np.float)*bhp
            for i in range(0,self.pipes.num):
...
#            #ax1.legend(loc='upper right', prop={'size':8}, ncol=1, numpoints=1)
    ...
    def detournay_visc(self,Q0=0.08):
        ...
        #time
        time = np.linspace(1000.0,200000.0,100,dtype=np.float)
        rho = np.linspace(0.001,0.999,100,dtype=np.float)
        drho = rho[1] - rho[0]
        wst = []
        pst = []
        rst = []
...
#                                int(np.random.uniform(0,30))],dtype=np.int) #count
...
#                                [r3,r3+np.random.uniform(100.0,800.0)]],dtype=np.float) #m
...
#                                [np.random.uniform(0.0,360.0)*deg,np.random.uniform(0.0,90.0)*deg]],dtype=np.float) #m
...
#                                [np.random.uniform(0.0,90.0)*deg,np.random.uniform(0.0,45.0)*deg]],dtype=np.float) #m
        geom.rock.fNum = np.asarray([int(np.random.uniform(0,30)),
...
#                                [np.random.uniform(0.0,90.0)*deg,np.random.uniform(0.0,45.0)*deg]],dtype=np.float) #m
        ...
        geom.rock.fDia = np.asarray([[10.0,100.0],
                                [10.0,100.0],
                                [10.0,100.0]],dtype=np.float) #m
        #FORGE rotated 104 CCW
        geom.rock.fStr = np.asarray([[15.0*deg,7.0*deg],
                                [260.0*deg,7.0*deg],
                                [120.0*deg,7.0*deg,]],dtype=np.float) #m
        geom.rock.fDip = np.asarray([[35.0*deg,7.0*deg],
                                [69.0*deg,7.0*deg,],
                                [35.0*deg,7.0*deg]],dtype=np.float) #m
        #fracture hydraulic parameters
#        r = np.random.exponential(scale=0.25,size=2)
#        r[r>1.0] = 1.0
#        r[r<0] = 0.0
