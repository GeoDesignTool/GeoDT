...
for i in range(0,2500):
    ...
    geom.rock.ResDepth = np.random.uniform(4000.0,10000.0) #6000.0 # m
    geom.rock.ResGradient = np.random.uniform(35.0,55.0) #50.0 #56.70 # C/km; average = 25 C/km
    ...
    geom.rock.Ks3 = np.random.uniform(0.5,0.9) #0.5
    ...
    #well parameters
    geom.rock.w_count = int(np.random.uniform(1,7)) #2 #production wells
    geom.rock.w_spacing = np.random.uniform(200.0,1100.0) #300.0 #m
    geom.rock.w_length = np.random.uniform(800.0,1200.0) #800.0 #m
    geom.rock.w_azimuth = geom.rock.s3Azn + np.random.uniform(0.0,90.0)*deg #rad
    geom.rock.w_dip = geom.rock.s3Dip + np.random.uniform(0.0,90.0)*deg #rad
    geom.rock.w_proportion = np.random.uniform(1.0,0.4) #0.8 #m/m
    ...
    geom.rock.w_intervals = int(np.random.uniform(1,11)) #3 #breaks in well length
    geom.rock.ra = 0.0254*np.random.uniform(4.0,9.0) #3.0 #0.0254*3.0 #m
    geom.rock.rgh = 80.0
...
#    geom.rock.Qinj = 0.01 #m3/s #this variable is changed in a unique way below since optimum flow rates are system specific
    ...
    rates = np.random.uniform(0.005,0.05,10)
    #rates = [0.01]
    for r in rates:
