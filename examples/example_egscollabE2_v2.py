...
#        #rock properties
    ...
    geom.rock.Ks2 = 0.99999 #1.05421 #geom.rock.Ks3 + np.random.uniform(0.4,0.6) # 0.75 #!!!
...
#        geom.rock.bh_bound = np.random.uniform(0.001,0.005)
    ...
    geom.rock.ra = 0.096 #m #0.1*0.0191 #limited by a_max, actual is 0.096 m for core hole or 0.0131 m for 3/4 HIP-MP tubing #!!!
    geom.rock.rgh = 80.0 #!!!
    #cement properties
...
#        geom.rock.mcc = np.asarray([np.min([u1,u2]),u3,np.max([u1,u2])]) #Pa
    ...
    for i in range(0,len(data)):
        if data['type'][i] == 1:
            if (data['depth_m'][i] > 30.0) and (data['depth_m'][i] < 71.0):
                c0 = [data['x_m'][i],data['y_m'][i],data['z_m'][i]]
                dia = dia
                strike = (data['azn_deg'][i]-90.0)*deg
    ...
    wells = []
    ra = 0.0131 #m #limit for flow solver stability at rate range of this model is 0.0191 m
    rc = geom.rock.ra #actual hole diameter, note that (1) ra of well is used for pipe flow and (2) rc of well is used for choke size
    rb = 0.5*(rc+ra)
    ...
    for s in range(0,5):
        ...
        #select interval        
        zone_leg = 0.65 #packer interval length
        comm.rock.sand = zone_leg #!!!
        rati_leg = np.random.uniform(0.45,0.85) #interval center depth
        ...
        #replace well diameters with suitable numbers for choke and internal pipe flow
        for w in wells2:
            w.ra = comm.rock.ra
            w.rb = w.ra + 0.001
            w.rc = w.rb + 0.001
        ...
        #stimulate
        comm.rock.Qstim = np.random.uniform(100.0*mLmin, 14000.0*mLmin) #0.08 #m3/s
        comm.rock.Vstim = np.random.uniform(1.0*gal, 500.0*gal) #100000.0 #m3
        ...
        rates = np.random.uniform(100.0*mLmin,14000.0*mLmin,5) 
        for r in rates:
    ...
    #                base.stim_and_flow(target=[],visuals=False,fname='run_%i' %(pin))
            ...
            x = base.save('inputs_results_collabE2.txt',pin,aux=[['inj_depth',rati_leg],['interval_length',zone_leg]],printwells=15)
