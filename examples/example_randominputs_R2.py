...
for i in range(0,1000):
    ...
    geom.rock.fDia = np.asarray([[50.0,1000.0],
                            [50.0,1000.0],
                            [50.0,1000.0]],dtype=float) #m
    ...
    #well parameters
    geom.rock.w_count = int(np.random.uniform(1,6)) #2 #production wells
    geom.rock.w_spacing = np.random.uniform(50.0,1000.0) #300.0 #m
    ...
    geom.rock.w_proportion = np.random.uniform(1.2,0.3) #0.8 #m/m
    ...
    geom.rock.w_intervals = int(np.random.uniform(1,9)) #3 #breaks in well length
