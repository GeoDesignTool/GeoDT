...
for i in range(0,250):
    ...
    geom.rock.ResDepth = np.random.uniform(7000.0,9400.0) #6000.0 # m
    geom.rock.ResGradient = np.random.uniform(45.0,55.0) #50.0 #56.70 # C/km; average = 25 C/km
    geom.rock.ResRho = np.random.uniform(2550.0,2850.0) #2700.0 # kg/m3
    geom.rock.ResKt = np.random.uniform(2.2,2.8) #2.5 # W/m-K
    geom.rock.ResSv = np.random.uniform(1850.0,2150.0) #2063.0 # kJ/m3-K
    geom.rock.AmbTempC = np.random.uniform(5.0,30.0) #25.0 # C
    geom.rock.AmbPres = 0.101 #Example: 0.01 MPa #Atmospheric: 0.101 # MPa
    geom.rock.ResE = np.random.uniform(50.0,110.0)*GPa #50.0*GPa
    geom.rock.Resv = np.random.uniform(0.15,0.35) #0.3
    geom.rock.Ks3 = np.random.uniform(0.6,0.9) #0.5
    geom.rock.Ks2 = geom.rock.Ks3 + np.random.uniform(0.1,0.30) # 0.75
    ...
    geom.rock.fDia = np.asarray([[50.0,np.random.uniform(100.0,1000.0)],
                            [50.0,np.random.uniform(100.0,1000.0)],
                            [50.0,np.random.uniform(100.0,1000.0)]],dtype=float) #m
    ...
    geom.rock.bh_max = 1.00 #0.02000 #m
    geom.rock.bh_bound = np.random.uniform(0.0001,0.002)
    geom.rock.f_roughness = np.random.uniform(0.7,1.0) #0.8
    
    #well parameters
    geom.rock.w_count = int(np.random.uniform(3,7)) #2 #production wells
    geom.rock.w_spacing = np.random.uniform(200.0,1100.0) #300.0 #m
    geom.rock.w_length = np.random.uniform(800.0,1200.0) #800.0 #m
    geom.rock.w_azimuth = geom.rock.s3Azn + np.random.uniform(0.0,45.0)*deg #rad
    geom.rock.w_dip = geom.rock.s3Dip + np.random.uniform(0.0,45.0)*deg #rad
    geom.rock.w_proportion = np.random.uniform(0.56,0.88) #0.8 #m/m
    ...
    geom.rock.w_intervals = int(np.random.uniform(6,14)) #3 #breaks in well length
    geom.rock.ra = 0.0254*np.random.uniform(5.0,7.5) #3.0 #0.0254*3.0 #m
    geom.rock.rgh = 80.0
    ...
    geom.rock.dPp = -1.0*np.random.uniform(-2.0,10.0)*MPa #-2.0*MPa #production well pressure difference from in-situ pore pressure
...
#    geom.rock.Qinj = 0.01 #m3/s #this variable is changed in a unique way below since optimum flow rates are system specific
    ...
    geom.rock.hfmcc = np.random.uniform(0.1,0.3)*MPa #0.1*MPa
    ...
    #stimulate
    geom.rock.Qstim = np.random.uniform(0.02,0.08,1)
    geom.rock.Vstim = np.random.uniform(0.1,6.0)*60.0*60.0*geom.rock.Qstim
    ...
    rates = np.random.uniform(0.030,0.046,10)
    #rates = [0.01]
    for r in rates:
