...
for i in range(0,1):
    ...
    geom.rock.ResGradient = np.random.uniform(30.0,60.0) #50.0 #56.70 # C/km; average = 25 C/km
    geom.rock.ResRho = np.random.uniform(2600.0,2800.0) #2700.0 # kg/m3
    ...
    geom.rock.fDia = np.asarray([[50.0,5000.0],
                            [50.0,5000.0],
                            [50.0,5000.0]],dtype=float) #m
    ...
    spread = np.random.uniform(0,1,(3))
    walk = np.random.uniform(-1,1,(3))
    ...
    geom.rock.bh = np.asarray([0.0005,0.001,0.002]) #np.asarray([0.00005,0.00010,0.00020])
    ...
    #well parameters
    geom.rock.w_count = int(np.random.uniform(1,5)) #2 #production wells
    geom.rock.w_spacing = np.random.uniform(100.0,800.0) #300.0 #m
    geom.rock.w_length = np.random.uniform(800.0,1200.0) #800.0 #m
    geom.rock.w_azimuth = geom.rock.s3Azn + np.random.uniform(0.0,0.0)*deg #rad
    geom.rock.w_dip = geom.rock.s3Dip + np.random.uniform(0.0,0.0)*deg #rad
    geom.rock.w_proportion = np.random.uniform(1.2,0.5) #0.8 #m/m
    geom.rock.w_phase = np.random.uniform(0.0,360.0)*deg #-90.0*deg #rad
    geom.rock.w_toe = np.random.uniform(-15.0,5.0)*deg #rad
    geom.rock.w_skew = np.random.uniform(-15.0,15.0)*deg #rad
    geom.rock.w_intervals = int(np.random.uniform(1,8)) #3 #breaks in well length
    geom.rock.ra = 0.0254*np.random.uniform(6.0,8.0) #3.0 #0.0254*3.0 #m
    geom.rock.rgh = 80.0
...
#    geom.rock.Qinj = 0.01 #m3/s #this variable is changed in a unique way below since optimum flow rates are system specific
    ...
    rates = np.random.uniform(0.005,0.08,1)
    #rates = [0.01]
    for r in rates:
