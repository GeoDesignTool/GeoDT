...
for i in range(0,1):
    ...
    geom.rock.perf_clusters = 4 #number of peforation clusters #!!!
    geom.rock.sand = 0.044 #0.044 #sand ratio in frac fluid by volume #!!!
    ...
    spacings = list(np.random.uniform(500.0,500.0,1)) #!!!
    for s in spacings:
        ...
        flows = list((10.0**(np.random.uniform(np.log10(0.08),np.log10(0.08),1)))/geom.rock.w_intervals) #!!!
        for f in flows:
